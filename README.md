...
The pipeline produces several attention scores measuring how much GWAS research attention each GBD condition has received. All scores are derived from the GWAS Catalog, matched to GBD conditions via EFO URIs in `Data/gbd_efo_master_mapping.tsv`. The 275-term GBD universe (non-injury leaf causes at levels 3/4, A/B outline) is defined in `Data/gbd_universe_275.csv`.

## Primary output: `Data/merged_dataset_exclude_Injuries.csv`

Generated by `scripts/pipeline.R` using functions in `scripts/pipeline_functions.R`.
...
## Extended output: `Data/GBD_combined_dataset_EFO.csv`
...
A **different** scoring formula used only within the canonical mapping review workflow — **not** currently used to produce `merged_dataset_exclude_Injuries.csv`:
...
- **Pipeline temporal** (`build_temporal_scores_from_master()` in `pipeline_functions.R`): produces `analysis_type ∈ {year, sliding_3yr}` rows in `merged_dataset_exclude_Injuries.csv`
...
`scripts/regional_concentration_index/` reads `total_attention_score` directly from `Data/merged_dataset_exclude_Injuries.csv` — no additional scoring is applied.
